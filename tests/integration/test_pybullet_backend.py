...
def make_backend() -> PyBulletBackend:
...
def test_backend_implements_contract_and_loads_named_joints() -> None:
    ...
    try:
        ...
        assert backend.body_position("palm").shape == (3,)
        assert backend.body_position("manipulation_object").shape == (3,)
        assert backend.body_position("target_zone").shape == (3,)
    finally:
        backend.close()
...
def test_reset_and_fixed_controls_are_deterministic() -> None:
    ...
    targets = JointTargets(
        {"thumb_flex": 0.3, "index_flex": 0.8, "middle_flex": 0.8, "ring_flex": 0.7},
        Command.CLOSE,
        timestamp_s=0.0,
    )
    try:
...
def test_invalid_target_constraint_and_shape_are_rejected() -> None:
    backend = make_backend()
    try:
...
def test_virtual_grasp_constraint_and_headless_rendering_work() -> None:
    backend = make_backend()
    try:
        backend.set_constraint_active("grasp_weld", active=True)
        backend.step(steps=2)
        frame = backend.render(width=160, height=120)
